...
void gui::GuiComponent::loadStyle(std::string _namestyle, std::string _type)
{
    ...
    std::filesystem::path fpath(ApplicationsFunctions::get_resources_dir().append("/styles").append(AppFiles::config_style));
    std::fstream fs;
    ...
    auto &txt = style["text"];
    if (txt["font"] == "default")
        mData.font = sf::Font(std::filesystem::path(ApplicationsFunctions::get_resources_dir().append(myConst::fonts::data_gameproces_font_path_3)));
    else
    {
        std::filesystem::path fpath(ApplicationsFunctions::get_resources_dir().append("/fonts").append(txt["font"].get<std::string>()));
