...
gui::ProgressBar::ProgressBar(sf::Vector2f pos, sf::Vector2f size,
  ...
  this->text.setPosition({this->background.getPosition().x +
                              (this->background.getSize().x * 0.5f) -
                              (this->text.getGlobalBounds().size.x * 0.5f),
                          this->background.getPosition().y +
                              (this->background.getSize().y * 0.5f) -
                              (this->text.getGlobalBounds().size.y * 0.5f)});
}

gui::ProgressBar::~ProgressBar() {}

// Functions
void gui::ProgressBar::update(const float current_value,
  ...
  current_value_str =
      current_value_str.substr(0, current_value_str.find(".") + 2);
  ...
  this->text.setPosition({this->background.getPosition().x +
                          ...
                          this->background.getPosition().y +
                              (this->background.getSize().y * 0.5f) -
                              (this->text.getGlobalBounds().size.y) +
                              mmath::p2pX(20, this->background.getSize().y)});
}
