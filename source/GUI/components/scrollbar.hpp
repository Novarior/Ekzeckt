...
private:
	...
	void updateThumbSize() {
		...
		if (this->sbType == SCROLLBAR_VERTICAL)
			this->thumb.setSize({this->track.getSize().x - 4.f, this->track.getSize().y * this->thumbRatio - 4.f});
		else
			this->thumb.setSize({this->track.getSize().x * this->thumbRatio - 4.f, this->track.getSize().y - 4.f});
	}
	...
	void updateThumbPosition() {
		...
		float trackLength = this->sbType == SCROLLBAR_VERTICAL ? this->track.getSize().y : this->track.getSize().x;

		float thumbLength = this->sbType == SCROLLBAR_VERTICAL ? this->thumb.getSize().y : this->thumb.getSize().x;
		...
		if (trackSpace > 0.f && this->contentSize > 0.f) {
			...
			if (this->sbType == SCROLLBAR_VERTICAL)
				this->thumb.setPosition({this->track.getPosition().x + 2.f, this->track.getPosition().y + 2.f + offset});
			else
				this->thumb.setPosition({this->track.getPosition().x + 2.f + offset, this->track.getPosition().y + 2.f});
		} else if (this->sbType == SCROLLBAR_VERTICAL)
			this->thumb.setPosition({this->track.getPosition().x + 2.f, this->track.getPosition().y + 2.f});
		else
			this->thumb.setPosition({this->track.getPosition().x + 2.f, this->track.getPosition().y + 2.f});
	}
