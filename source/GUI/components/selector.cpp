...
namespace gui
{
        ...
        : keytime(0.f), keytimeMax(0.3f), text(font, "", character_size)
    {
        ...
        // init buttons
        this->buttons["left"] = new gui::Button(sf::Vector2f(pos.x, pos.y), sf::Vector2f(mmath::p2pX(15, size.x), size.y), "-", gui::styles::buttons::btn_selector, gui::type::BUTTON);

        this->buttons["right"] = new gui::Button(sf::Vector2f(this->box.getPosition().x + this->box.getSize().x, pos.y), sf::Vector2f(mmath::p2pX(15, size.x), size.y), "+", gui::styles::buttons::btn_selector, gui::type::BUTTON);
