...
Selector::Selector(sf::Vector2f pos, sf::Vector2f size, sf::Font& font,
	...
	// init box
	this->box.setPosition({pos.x + (mmath::p2pX(10.f, size.x)), pos.y});
	this->box.setSize(sf::Vector2f(size.x - (mmath::p2pX(20.f, size.x)), size.y));
	...
	// init buttons
	this->buttons["left"] = new gui::Button(sf::Vector2f(pos.x, pos.y), sf::Vector2f(mmath::p2pX(15.f, size.x), size.y), "-", gui::styles::buttons::btn_selector, gui::type::BUTTON);

	this->buttons["right"] = new gui::Button(sf::Vector2f(this->box.getPosition().x + this->box.getSize().x, pos.y), sf::Vector2f(mmath::p2pX(15.f, size.x), size.y), "+", gui::styles::buttons::btn_selector, gui::type::BUTTON);

	// init text
	this->text.setString(list[0]);
	this->text.setPosition({this->box.getPosition().x + (this->box.getSize().x / 2.f - this->text.getGlobalBounds().size.x / 2.f), this->box.getPosition().y + this->text.getGlobalBounds().size.y / 2.f});
