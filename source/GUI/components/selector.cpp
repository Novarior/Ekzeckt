...
Selector::Selector(sf::Vector2f pos, sf::Vector2f size, sf::Font& font,
	...
	// init buttons
	buttons["left"] = new gui::Button(sf::Vector2f(pos.x, pos.y), sf::Vector2f(mmath::p2pX(15.f, size.x), size.y), "-");

	buttons["right"] = new gui::Button(sf::Vector2f(box.getPosition().x + box.getSize().x, pos.y), sf::Vector2f(mmath::p2pX(15.f, size.x), size.y), "+");
...
void Selector::update(const float& delta_time, const sf::Vector2f& mousePos) {
	for (auto& it : buttons)
