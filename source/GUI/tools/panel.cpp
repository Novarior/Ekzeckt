...
void gui::Panel::update(const sf::Vector2i &mousePosWindow, const float &dt)
{
    ...
    if (this->animating)
    {
        ...
        if (progress >= 1.f)
        {
            ...
            this->fadeAlpha = this->closing ? 255.f * (1.f - progress) : 255.f * progress;
    ...
    if (this->draggable)
    {
        ...
        if (!this->dragging && sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && this->titleBar.getGlobalBounds().contains(mousePos))
        {
