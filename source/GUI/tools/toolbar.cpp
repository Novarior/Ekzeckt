...
// gui::ToolBar::ToolBar(sf::Vector2f position, short unsigned orientation, float button_size, float padding, float spacing)
//     : orientation(orientation), buttonSize(button_size), padding(padding), spacing(spacing), visible(true)
// {
...
//         this->background.setSize({this->padding * 2.f, this->padding * 2.f + this->buttonSize});
//     else

//         this->background.setSize({this->padding * 2.f + this->buttonSize, this->padding * 2.f});
// }
...
// // Управление кнопками
// void gui::ToolBar::addButton(const std::string &key, const std::string &text, sf::Font &font, const std::function<void()> &callback)
// {
...
//         if (this->orientation == TOOLBAR_HORIZONTAL)
//             currentOffset += this->buttons.size() * (this->buttonSize + this->spacing);
//         else
//             currentOffset += this->buttons.size() * (this->buttonSize + this->spacing);

//     sf::Vector2f buttonPos;
//     if (this->orientation == TOOLBAR_HORIZONTAL)
//         buttonPos = {this->background.getPosition().x + currentOffset, this->background.getPosition().y + this->padding};
//     else
//         buttonPos = {this->background.getPosition().x + this->padding, this->background.getPosition().y + currentOffset};

//     // Создаем новую кнопку с базовыми цветами
//     gui::Button *button = new gui::Button(buttonPos, sf::Vector2f(this->buttonSize, this->buttonSize), text, gui::styles::buttons::btn_tools, gui::type::BUTTON);
