...
void Core::initDirectories()
{
    if (ApplicationsFunctions::checkAppDirectoryExists())
        Logger::logInfo("App directory already exists", "Core::initDirectories()");
    else
...
void Core::initVariabless()
{
    ...
    if (!ifs.is_open())
        Logger::logError("CANNOT OPEN FILE", "Core::initVariables()");
    else
    {
        try
        {
        ...
        {
            Logger::logError("JSON::PARSE_ERROR: " + std::string(e.what()),
                             "Core::initVariables()");
    ...
    auto result = cr_LoadData(cr_databuffer, *cr_gfxSettings, *cr_KeyBinds,
                              *cr_VolumeCollector);
    if (!result)
        Logger::logError("Failed to load keybinds!", "Core::initVariables()");
    ...
    if (!cr_GameFont_basic.openFromFile(std::string(ApplicationsFunctions::get_resources_dir() + myConst::fonts::data_gameproces_font_path)))
        Logger::logInfo("Debug font failed to load", "Core::initVariables()");
...
{
    // null текстура
    TextureManager::loadTexture(TextureID::TEXTURE_NULL, myConst::textures::texture_NULL);
    // текстуры для карты
    ...
    TextureManager::loadTexture(TextureID::ITEMS_STONE, ItemTextures::items::item_img_stone);
    // Дополнительные текстуры для новых предметов
    // Эти текстуры должны быть добавлены в resources, пока используем
    // существующие
    TextureManager::loadTexture(TextureID::ITEMS_WOOD, ItemTextures::items::item_img_stone2);                              // Временно используем текстуру камня
    TextureManager::loadTexture(TextureID::ITEMS_IRON_SWORD, ItemTextures::items::item_img_claster_crystal);               // Временно используем текстуру кристалла
    TextureManager::loadTexture(TextureID::ITEMS_BREAD, ItemTextures::item_NULL);                                          // Временно используем пустую текстуру
...
{ // send window state stack and fonts to state data
    ...
    cr_Statedata.sd_characterSize_debug = mmath::calcCharSize(cr_Window->getSize(), 150);
    cr_Statedata.sd_characterSize_game_big = mmath::calcCharSize(cr_Window->getSize(), 60);
    cr_Statedata.sd_characterSize_game_medium = mmath::calcCharSize(cr_Window->getSize(), 85);
    cr_Statedata.sd_characterSize_game_small = mmath::calcCharSize(cr_Window->getSize(), 100);
    ...
    if (!cr_Statedata.sd_Window.lock())
        Logger::logError("ERROR::WINDOW::NOT INITED", "Core::initStateData()");

    // check if states is not empty or null idk
    if (!cr_Statedata.sd_States->empty())
        Logger::logError("ERROR::STATES::NOT INITED", "Core::initStateData()");
#endif
}
...
void Core::initState()
{
    ...
    Logger::logInfo("State inited", "Core::initState()");
    Logger::logInfo("State size: " + std::to_string(cr_State.size()),
                    "Core::initState()");
#endif
}
...
#if __MDEBUG__ == 1
    Logger::logInfo("Core Inited", "Core::Core()");
#endif
}
...
#if __MDEBUG__ == 1
    // logger moment
    Logger::logInfo("Core Delete...", "Core::~Core()");

    if (cr_State.empty())
        Logger::logInfo("State is empty", "Core::~Core()");
    else
        Logger::logError("State is not empty... mem leaked", "Core::~Core()");

    if (cr_Window == NULL)
        Logger::logInfo("Window is null", "Core::~Core()");
    else
        Logger::logError("Window is null... mem leaked", "Core::~Core()");

    Logger::logInfo("Core Deleted", "Core::~Core()");
#endif
}

void Core::run()
{
    Logger::logInfo("Start main loop", "Core::run()");
