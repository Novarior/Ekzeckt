...
{ // init data who dont use loaded dates
    IKeyBinds.lock()->at("KEY_CLOSE") = IstateData->sd_KeySupports.lock()->at("Escape");
    IKeyBinds.lock()->at("KEY_TAB") = IstateData->sd_KeySupports.lock()->at("Tab");
    ...
    IKeyBinds.lock()->at("KEY_E") = IstateData->sd_KeySupports.lock()->at("E");
    IKeyBinds.lock()->at("KEY_Q") = IstateData->sd_KeySupports.lock()->at("Q");
    IKeyBinds.lock()->at("KEY_SPACE") = IstateData->sd_KeySupports.lock()->at("Space");
...
{
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_TAB")) && this->getKeytime())
        this->player->e_getInventory()->toggleInventory();
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_CLOSE")) && this->getKeytime())
...
{
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_A")))
        this->player->e_move(-1.f, 0.f, delta_time);
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_D")))
        this->player->e_move(1.f, 0.f, delta_time);
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_S")))
        this->player->e_move(0.f, 1.f, delta_time);
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_W")))
        this->player->e_move(0.f, -1.f, delta_time);

    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_SPACE")) && this->getKeytime())
    {
...
void Process::renderGUI(sf::RenderTarget &target)
{
    ...
    if (this->Ipaused) // Pause menu render
        ...
        else if (this->player->e_getInventory()->isInventoryOpened() && this->inventoryGUI)
