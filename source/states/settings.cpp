...
{ // init sd_font
    this->font = this->IstateData->sd_GameFont_basic;
}

void SettingsState::initKeybinds()
{ // init key escape like defoult back button
    IKeyBinds.lock()->at("KEY_BACK") = IstateData->sd_KeySupports.lock()->at("Escape");
    IKeyBinds.lock()->at("KEY_SLASH") = IstateData->sd_KeySupports.lock()->at("Slash");
    IKeyBinds.lock()->at("KEY_BACK_PAGE") = IstateData->sd_KeySupports.lock()->at("Q");
    IKeyBinds.lock()->at("KEY_FORWARD_PAGE") = IstateData->sd_KeySupports.lock()->at("E");
    // IKeyBinds.lock()->at("KEY_A") = IstateData->sd_KeySupports.lock()->at("A");
    // IKeyBinds.lock()->at("KEY_W") = IstateData->sd_KeySupports.lock()->at("W");
    // IKeyBinds.lock()->at("KEY_S") = IstateData->sd_KeySupports.lock()->at("S");
    // IKeyBinds.lock()->at("KEY_D") = IstateData->sd_KeySupports.lock()->at("D");
}
...
{ // init gui with next call functions
    ...
    _pageBackground.setSize(sf::Vector2f(mmath::p2pX(70, window_size.x), mmath::p2pX(66, window_size.y)));
    ...
    _pageBackground.setPosition(sf::Vector2f( // position of page background
        ...
        mmath::p2pX(50, window_size.y) - _pageBackground.getSize().y / 2));
    _pageBackground.setFillColor(sf::Color(140, 140, 140, 140));

    sf::Vector2f button_size = sf::Vector2f(mmath::p2pX(16, window_size.x), mmath::p2pX(5, window_size.y));
...
void SettingsState::initGraphicsPage()
{
    ...
    sf::Vector2f background_layer_size = sf::Vector2f(
        mmath::p2pX(70, window_size.x), mmath::p2pX(66, window_size.y));
    sf::Vector2f button_size =
        sf::Vector2f(mmath::p2pX(20, background_layer_size.x), mmath::p2pX(5, background_layer_size.y));
...
{ // init sound page
    ...
    float slidersizeY = mmath::p2pX(10, _pageBackground.getSize().y);
    ...
    _sound_SliderMap[gfx::SoundCategory::vol_MASTER] =
        std::make_unique<gui::SliderInt>( // init MASTER slider
            sf::Vector2f(offset_position_for_sliders.x + mmath::p2pX(50, background_size.x), offset_position_for_sliders.y),
            slider_size, font,
            IstateData->sd_VolumeCollector.lock()->getCategoryVolume(gfx::SoundCategory::vol_MASTER),
            0, 100, 1, this->IstateData->sd_characterSize_game_medium, helperText::VolumeTexts::VOL_MASTER);

    _sound_SliderMap[gfx::SoundCategory::vol_SFX] =
        std::make_unique<gui::SliderInt>( // init SFX slider
            sf::Vector2f(offset_position_for_sliders.x + mmath::p2pX(50, background_size.x),
...
{ // init keyboard page
    sf::Vector2u window_size = this->Iwindow.lock()->getSize();
    sf::Vector2f button_size = sf::Vector2f(mmath::p2pX(16, window_size.x), mmath::p2pX(5, window_size.y));
    sf::Text text(this->IstateData->sd_GameFont_basic, "", 24);
    sf::RectangleShape rectangle;
    rectangle.setFillColor(sf::Color(200, 200, 200, 150));

    int i = 0;
    float i2 = i;
    for (const auto &keybind : *this->IstateData->sd_KeySupports.lock())
    {
        text.setString(keybind.first + ": " + std::to_string(static_cast<int>(keybind.second)));

        float posX = mmath::p2pX(12.f * (i % 3), window_size.x);
        float posY = mmath::p2pX(40.f, window_size.y) + mmath::p2pX(3.f * (i2 / 3), window_size.y);
...
void SettingsState::resetGui()
{
    ...
    gfx::myGFXStruct gfx = *this->IstateData->sd_gfxSettings.lock();
    ...
    // reset window
    if (gfx.fullscreen)
        this->Iwindow.lock()->create(gfx.resolution, gfx.title, sf::State::Fullscreen, gfx.contextSettings);
    else
        this->Iwindow.lock()->create(gfx.resolution, gfx.title, sf::State::Windowed, gfx.contextSettings);

    this->Iwindow.lock()->setFramerateLimit(gfx.frameRateLimit);

    // save all data settings to file
    // add later
    // this->IstateData->sd_gfxSettings.lock()->saveToFile();
    this->IstateData->sd_reserGUI = true;
...
SettingsState::SettingsState(StateData *state_data)
    : State(state_data), page(settingPage::GRAPHICS), pageName("GRAPHICS")
{ // init variables
    initPageLayout();
    initVariables();
    initFonts();
...
void SettingsState::updateInput(const float &delta_time)
{
    ...
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_BACK")) && this->getKeytime())
        this->endState();
    // if slash is pressed, swith debug mode
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_SLASH")) && this->getKeytime())
        this->Idebud = !this->Idebud;

    // if key Q is pressed, swith page to left (back)
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_BACK_PAGE")) && this->getKeytime())
    ...
    if (IKeyboard.lock()->isKeyPressed(IKeyBinds.lock()->at("KEY_FORWARD_PAGE")) && this->getKeytime())
...
{
    // this->Isound.setVolume(this->IVolumeCollector->getCategoryVolume(gfx::SoundCategory::vol_MASTER)
    //    * this->IVolumeCollector->getCategoryVolume(gfx::SoundCategory::vol_MUSIC) /
    //    100);
}
...
void SettingsState::updateGui(const float &delta_time)
{
    ...
    if (this->Idebud)
    {
        this->IstringStream
            ...
            << std::to_string(this->IstateData->sd_gfxSettings.lock()->resolution.size.x)
            << std::to_string(this->IstateData->sd_gfxSettings.lock()->resolution.size.y)
            << "\nFramerate limit: " + std::to_string(this->IstateData->sd_gfxSettings.lock()->frameRateLimit)
            << "\nResolution: " << this->IstateData->sd_Window.lock()->getSize().x
            << "x" << this->IstateData->sd_Window.lock()->getSize().y
            << "\nAntialiasing: "
            << this->IstateData->sd_gfxSettings.lock()->contextSettings.antiAliasingLevel
            << "\nvSync: " << this->IstateData->sd_gfxSettings.lock()->verticalSync
            << "\nFullscreen: "
            << this->IstateData->sd_gfxSettings.lock()->fullscreen
